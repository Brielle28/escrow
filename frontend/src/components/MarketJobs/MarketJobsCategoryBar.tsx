...
type MarketJobsCategoryBarProps = {
  category: JobCategory | "all";
  onCategoryChange: (c: JobCategory | "all") => void;
  resultCount: number;
};

export function MarketJobsCategoryBar({ category, onCategoryChange, resultCount }: MarketJobsCategoryBarProps) {
  const filters = categoryFilters();

  return (
    <div className="mt-10 border-b border-gray-100 pb-8">
      <p className="text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-gray-400">{marketJobsPageCopy.categoriesLabel}</p>
      <div className="mt-2 flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2" role="group" aria-label="Category filters">
        ...
        <p className="shrink-0 text-sm font-medium tabular-nums text-gray-600">{marketJobsPageCopy.resultsLabel(resultCount)}</p>
      </div>
    </div>
  );
}
