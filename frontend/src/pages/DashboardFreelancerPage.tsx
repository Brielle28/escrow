...
export function DashboardFreelancerPage() {
  const session = loadWalletSession();

  return (
    <LandingLayout>
      <RequireWalletSession role="freelancer">
      <section className="mx-auto max-w-3xl px-6 py-16">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-600">Freelancer</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-gray-900">Dashboard</h1>
        <p className="mt-3 text-gray-600">
          You&apos;re signed in as a <strong className="text-gray-900">Freelancer</strong>
          {session ? (
            <>
              {" "}
        ...
        <p className="mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          This screen is a placeholder. Wire job board and applications here next.
        </p>
